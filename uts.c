...
void uts_initRoot(UTSConfig *c, Node * root) {
  ...
  rng_init(root->state.state, c->rootId);
...
int uts_numChildren_bin(UTSConfig *c, Node * parent) {
  ...
  return (d < c->nonLeafProb) ? c->nonLeafBF : 0;
}
...
int uts_numChildren_geo(UTSConfig *c, Node * parent) {
  ...
  if (depth > 0){
    ...
    case EXPDEC:
      b_i = c->b_0 * pow((double) depth, -log(c->b_0)/log((double) c->gen_mx));
      break;
    ...
    case CYCLIC:
      ...
      b_i = pow(c->b_0,
                sin(2.0*3.141592653589793*(double) depth / (double) c->gen_mx));
      break;

      // identical distribution at all nodes up to max depth
    case FIXED:
      b_i = (depth < c->gen_mx)? c->b_0 : 0;
      break;

      // linear decrease in b_i
    case LINEAR:
    default:
      b_i =  c->b_0 * (1.0 - (double)depth / (double) c->gen_mx);
      break;
    }
  }
...
int uts_numChildren(UTSConfig *c, Node *parent) {
  ...
  switch (c->type) {
    case BIN:
      if (parent->height == 0)
        numChildren = (int) floor(c->b_0);
      else
        numChildren = uts_numChildren_bin(c, parent);
      break;

    case GEO:
      numChildren = uts_numChildren_geo(c, parent);
      break;

    case HYBRID:
      if (parent->height < c->shiftDepth * c->gen_mx)
        numChildren = uts_numChildren_geo(c, parent);
      else
        numChildren = uts_numChildren_bin(c, parent);
      break;
    case BALANCED:
      if (parent->height < c->gen_mx)
        numChildren = (int) c->b_0;
      break;
    default:
...
void uts_parseParams(UTSConfig *c, int argc, char *argv[]){
  ...
  while (i < argc && err == -1) {
    ...
    switch (argv[i][1]) {
      case 'q':
        c->nonLeafProb = atof(argv[i+1]); break;
      case 'm':
        c->nonLeafBF = atoi(argv[i+1]); break;
      case 'r':
        c->rootId = atoi(argv[i+1]); break;
      case 'x':
        c->debug = atoi(argv[i+1]); break;
      case 'v':
        c->verbose = atoi(argv[i+1]); break;
      case 't':
        c->type = (tree_t) atoi(argv[i+1]);
        if (c->type != BIN && c->type != GEO && c->type!= HYBRID && c->type != BALANCED)
	  err = i;
        break;
      case 'a':
        c->shape_fn = (geoshape_t) atoi(argv[i+1]);
        if (c->shape_fn > FIXED) err = i;
        break;
      case 'b':
        c->b_0 = atof(argv[i+1]); break;
      case 'd':
        c->gen_mx = atoi(argv[i+1]); break;
      case 'f':
        c->shiftDepth = atof(argv[i+1]); break;
      case 'g':
        c->computeGranularity = max(1,atoi(argv[i+1])); break;
      default:
        err = i;
    }
...
void uts_showStats(UTSConfig *c, int nPes, int chunkSize, double walltime, counter_t nNodes, counter_t nLeaves, counter_t maxDepth) {
  ...
  if (c->verbose == 0) {
    printf("%4d %7.3f %9llu %7.0llu %7.0llu %d %d %.2f %d %d %1d %f %3d\n",
        ...
        c->type, c->b_0, c->rootId, c->gen_mx, c->shape_fn, c->nonLeafProb, c->nonLeafBF);
  }
